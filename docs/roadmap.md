...
5. [x] **Valider par replay et comparaison visuelle.** Les épisodes court (53), médian (5) et long (26) ont été rejoués à 30 Hz logique (deux pas de simulation à 60 Hz par action), sans reset ; les planches caméra et graphiques ont été revus. Le replay reste non déterministe si l'état initial de scène manque. Hors périmètre : contourner un mismatch par reshape ou normalisation arbitraire.
6. [ ] **Entraîner ACT sur le dataset validé — smoke test terminé.** Le test de 20 étapes a effectué forward/backward, créé les checkpoints `000010` et `000020`, puis rechargé `last`. Reste à préparer, sans lancer automatiquement, l'entraînement RTX 3090 plus long avec losses locales. Hors périmètre : entraînement long automatique ou autres politiques.
