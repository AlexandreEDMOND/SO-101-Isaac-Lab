...
5. [ ] **Valider par replay et comparaison visuelle — en cours.** L'épisode 0 a été rejoué à 30 Hz logique (deux pas de simulation à 60 Hz par action), sans reset ; la planche caméra et le graphique de trajectoires attendent une revue visuelle. Le replay reste non déterministe si l'état initial de scène manque. Hors périmètre : contourner un mismatch par reshape ou normalisation arbitraire.
